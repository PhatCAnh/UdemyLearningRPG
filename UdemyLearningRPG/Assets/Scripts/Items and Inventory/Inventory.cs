...
public class Inventory : MonoBehaviour
{
    ...
    {
        RemoveFromCollection(item, inventoryDictionary, inventory);
        RemoveFromCollection(item, stashDictionary, stash);
        UpdateSlotUI();
    }

    private void RemoveFromCollection(ItemData item, Dictionary<ItemData, InventoryItem> dictionary, List<InventoryItem> collection)
    {
        if (dictionary.TryGetValue(item, out InventoryItem inventoryItem))
        {
            if (inventoryItem.stackSize <= 1)
            {
                collection.Remove(inventoryItem);
